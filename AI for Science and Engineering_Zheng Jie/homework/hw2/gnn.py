...
class QSARGNN(nn.Module):
    def __init__(self, in_feats, hidden_size, num_classes, num_heads=4):
        super(QSARGNN, self).__init__()
        # 使用GATConv作为图注意力层
        self.gat1 = GATConv(in_feats, hidden_size, num_heads=num_heads)
        self.gat2 = GATConv(hidden_size * num_heads, hidden_size, num_heads=num_heads)
        # Additional GATConv layers
        self.gat3 = GATConv(hidden_size * num_heads, hidden_size, num_heads=num_heads)
        self.gat4 = GATConv(hidden_size * num_heads, hidden_size, num_heads=num_heads)
        self.classify = nn.Linear(hidden_size * num_heads, num_classes)
    ...
    def forward(self, g):
        ...
        h = torch.relu(h)
        h = self.gat3(g, h).flatten(1)  # Additional layer
        h = torch.relu(h)
        h = self.gat4(g, h).flatten(1)  # Additional layer
        h = torch.relu(h)
        g.ndata['h'] = h
